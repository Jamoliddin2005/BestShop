...
exports.usersGet = async (req, res) => {
    try {
        ...
        if (!users) {
            return res.status(200).json({ success: true, message: "Users not found!" })
        }
...
exports.PostPasswordSubmit = async (req, res) => {
    const user = await Users.find({ phoneNumber: req.body.phoneNumber })
    const bcCompare = await bcrypt.compare(req.body.password, user[0].password)
    if (!bcCompare) {
